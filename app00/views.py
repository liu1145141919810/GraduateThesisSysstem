...
def login(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            status=tb_login.objects.get(username=user).usage_state
            if not status:
                tb_login_log.objects.create(username=login_record, status='fail')
                return render(request, "login.html", {'error': '登录失败：账号已被禁用'})
             # 比对解密后的密码
            if stored_pwd == pwd:
...
def index_page(request, page):
    ...
    if role==1:
        ...
        username=tb_login.objects.get(id=request.session.get('user_id')).username
    ...
    if  page == "manager":
        # 权限控制：仅管理员可访问（role=0 为管理员）
        if role != 0:
            request.session['error'] = "权限不足：仅管理员可访问"
            return redirect("/index/")
        # 获取管理员页面所需数据
        return manager_it(request,template_name,username)
        users = tb_login.objects.all()  # 所有用户信息
        login_logs = tb_login_log.objects.all().order_by('-log_time')[:10]  # 最近10条登录日志
        issues = tb_topic.objects.all()  # 所有选题数据
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            if user.role == 1:
                ...
                teacher, created = tb_student.objects.get_or_create(email=user.email)
                
            teacher.display_name = data.get('display_name', teacher.display_name)
            teacher.phone = data.get('phone', teacher.phone)
            teacher.college = data.get('college', teacher.college)
...
def review_user(request, user_id):
    ...
    try:
        user = tb_login.objects.get(id=user_id)
        # 这里可以添加审核逻辑，例如更新审核状态字段
        user.is_active = True  # 审核通过后启用账号
        user.save()
        return JsonResponse({'status': 'success'})
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=400)
